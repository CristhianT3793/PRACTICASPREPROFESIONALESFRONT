...
namespace FPP_front
{
    public partial class informesemestralfinal : System.Web.UI.Page
    {
        static readonly Servicios con = new Servicios();
        static int idplatillaFpp7 = -1;
            ...
            if (!IsPostBack) {
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        cargarInformeSemestral(1, identificacioncoordinador, carreras);//cedula de un coordinador
                    }
                }
                catch (Exception ex)
                {
        ...
        public async void cargarInformeSemestral(int pagina,string identificacionCoordinador, string[] carreras_)
        {
            ...
            string uri = "FppCoordinador/GetFppCoordinadorCarrera/page/" + pagina + "/carrera=" + carreras_separado + "/tipoDocumento=FPP4";
            ...
            if (micro_getdatos != "error")
            {
                ...
                if (Convert.ToBoolean(hasitems))
                {
                    dgvFppInformeSemestral.VirtualItemCount = Convert.ToInt32(total);
                    dgvFppInformeSemestral.DataSource = plantilla_.ToList();
                    dgvFppInformeSemestral.DataBind();
                }
            }
        }
        public async void cargarplanificacionxParametro(int pagina, string identificacionCoordinador, string[] carreras_, string parametro)
        {
            ...
            carreras_separado = carreras_separado.Substring(1, carreras_separado.Length - 1);
            string uri = "FppCoordinador/GetFppCoordinadorCarrera/page/" + pagina + "/carrera=" + carreras_separado + "/tipoDocumento=FPP4/parametro=" + parametro;
            List<DTOFppCoordinador> plantilla_ = new List<DTOFppCoordinador>();
            string micro_getdatos = string.Empty;
            micro_getdatos = await con.GenericGet(uri);
            if (micro_getdatos != "error")
        ...
        {
            DataSet ds_parametro = Conexion.BuscarPracticas_ds("FPP_COORDINADOR", "*", "where NOM_PLATILLA_CORDINADOR='FPP4' AND CARRERA_FPP_COORDINADOR='" + codcarrera + "' AND SEMESTRE_FPP_COORDINADOR='" + semestre + "'");
        ...
        protected async void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            fppcoordinador.IdPlantilla = idplatillaFpp7;
            fppcoordinador.FechaRegistroFpp = DateTime.Now;
            fppcoordinador.NomPlatillaCordinador = "FPP4";
            ...
            if (!validarParametro(ddlcarrera.SelectedValue, ddlsemestre.SelectedValue))
            {
                if (file_.HasFile && file_.Enabled)
                {
                    ...
                    nombreArchivo = "FPP4_INFORME_FINALIZACION_PRACTICAS_"+ddlcarrera.SelectedValue.Trim()+"_"+ ddlsemestre.SelectedValue + System.IO.Path.GetExtension(strFileNameWithPath);
                    ...
                    if (validarArchivo(nombreArchivo,file_))//valida que el documento se haya guardado correctamente
                    {
                        ...
                        if (correcto)
                        {
                            ...
                            cargarInformeSemestral(1, identificacioncoordinador, carreras);
                            //getPlantilla(1);
                            //limpiarCampos();
                        }
                        else
        ...
        //falta verificar la profundidad del parametro enviar carrera y semestre
        public async Task<string> ServicioExtarerPathPlantilla(string TipoDocumento)
        {
            ...
            if (micro_getdatos != "error")
            {
                ...
                if (Convert.ToBoolean(hasitems))
                {
                    if (plantilla_.Select(x => x.NomrePlantilla.Trim()).Contains(TipoDocumento.Trim()))
                    {
                        
                        idplatillaFpp7 = plantilla_.Where(x=>x.NomrePlantilla.Trim()==TipoDocumento.Trim()).Select(x => x.IdPlantilla).FirstOrDefault();//poner aqui en un where y demas campos la profundidad
                        if (idplatillaFpp7 != -1)
                        {
                            return pathfpp= plantilla_.Where(x=>x.IdPlantilla==idplatillaFpp7).Select(x => x.PathPlatilla).FirstOrDefault();
                        }else
                        {
                            return pathfpp;
                        }
        ...
        protected void dgvFppInformeSemestral_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            int fila = Convert.ToInt32(e.CommandArgument);
            if (e.CommandName == "descargarInforme")
            {
                string path = dgvFppInformeSemestral.DataKeys[fila]["PathFppCoordinador"].ToString();
        ...
        protected void ddlcarrera_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (ddlcarrera.SelectedValue != "0")
            {
                ...
                if (idProfundidad_ != -1)
                {
                    ...
                    cargarplantillaFPP7(idProfundidad_, tipoFpp);
                    //cargar plantilla
                }
                else
                {
        ...
        public async void cargarplantillaFPP7(int idprofundidad, string tipofpp)
        {
        ...
        protected void dgvFppInformeSemestral_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            dgvFppInformeSemestral.PageIndex = e.NewPageIndex;
            int page = e.NewPageIndex + 1;
            cargarInformeSemestral(page, identificacioncoordinador, carreras);
        }
